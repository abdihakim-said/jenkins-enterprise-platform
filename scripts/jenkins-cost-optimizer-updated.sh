#!/bin/bash
# Jenkins Enterprise Cost Optimization System - Updated
# Now uses intelligent build management instead of basic cleanup
...
# Get Jenkins metrics (same as before)
get_jenkins_metrics() {
...
# Get current infrastructure costs (same as before)
get_current_costs() {
...
# Intelligent scaling decision (same as before)
...
# Execute scaling action (same as before)
execute_scaling() {
...
# Calculate cost impact (same as before)
calculate_cost_impact() {
    local from_capacity=$1
    local to_capacity=$2
    local spot_price=0.012
...
# Store scaling event (same as before)
store_scaling_event() {
    local from_capacity=$1
    local to_capacity=$2
    local reason=$3
...
# Get Jenkins metrics as JSON (same as before)
...
# Get cost metrics as JSON (same as before)
...
# Check if off-hours (same as before)
is_off_hours() {
    local hour=$(date +%H)
    local day=$(date +%u)
...
# UPDATED: Use enterprise build management instead of basic cleanup
run_enterprise_build_management() {
    log "${YELLOW}🏢 Running enterprise build management...${NC}"
    
    # Call the enterprise build manager script
    if [ -f "/opt/jenkins/scripts/jenkins-build-manager.sh" ]; then
        bash /opt/jenkins/scripts/jenkins-build-manager.sh
    elif [ -f "./scripts/jenkins-build-manager.sh" ]; then
        bash ./scripts/jenkins-build-manager.sh
    else
        log "${YELLOW}⚠️ Enterprise build manager not found, using basic cleanup${NC}"
    ...
    log "${GREEN}✅ Build management completed${NC}"
}

# Generate cost report (same as before)
generate_cost_report() {
    local report_type=$1
    local report_file="$COST_REPORTS_DIR/${report_type}-$(date +%Y%m%d).json"
    
    local baseline_cost=300
    ...
    cat > "$report_file" << EOF
{
  ...
  "optimization_features": {
    ...
    "enterprise_build_management": "enabled",
    "intelligent_artifact_backup": "enabled"
  },
...
EOF
    ...
    aws s3 cp "$report_file" "s3://$S3_BUCKET/${report_type}-reports/$(basename $report_file)" && \
        log "${GREEN}📊 ${report_type} cost report uploaded to S3${NC}" || \
        log "${RED}⚠️ Failed to upload ${report_type} report${NC}"
    
    rm -f "$report_file"
}

# Send cost alert (same as before)
send_cost_alert() {
    local current_monthly=$(get_current_costs | cut -d',' -f5)
    local budget_limit=100
...
main() {
    log "${GREEN}🚀 Starting Jenkins Enterprise Cost Optimization - $(date)${NC}"
    
    # Core optimization
    execute_scaling
    
    # UPDATED: Use enterprise build management
    run_enterprise_build_management
    ...
    log "${BLUE}📊 Status: Queue=$queue_length, Workers=$current_capacity, Monthly=\$$(printf '%.2f' $monthly_cost)${NC}"
    log "${GREEN}✅ Enterprise cost optimization completed${NC}"
}
